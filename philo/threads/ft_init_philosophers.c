...
/*   Updated: 2024/06/25 12:50:35 by yel-moun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_philosophers(t_general_info *info)
{
	...
	while (i < info->philo_num)
	{
		...
		if (!info->philosophers[i].last_meal_lock || !info->philosophers[i].meal_count_lock)
			return (1);
		if (pthread_mutex_init(info->philosophers[i].last_meal_lock, NULL) != 0 
			|| pthread_mutex_init(info->philosophers[i].meal_count_lock, NULL) != 0)
			return (1);
